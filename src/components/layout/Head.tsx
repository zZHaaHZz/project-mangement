    ...
    <header className="flex justify-between items-center p-4 border-b border-gray-300 bg-white px-8">
      ...
      <div className="flex items-center gap-8">
        ...
        <Badge count={0} showZero={false}>
          <BellOutlined className="text-3xl cursor-pointer hover:text-blue-600" />
        </Badge>
        <div className='w'>
        {user && (  
          <span className="text-gray-700  md:block text-3xl font-bold">
            {user.name}
          </span>
        )}
        {user && (
          <span className="text-gray-700  md:block text-1xl font-bold">
            {user.role}
          </span>
        )}
        </div>
